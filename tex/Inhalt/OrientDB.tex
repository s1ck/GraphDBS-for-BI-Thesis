...
Im Gegensatz zu HyperGraphDB folgt die Implementierung der Lesezugriffe nicht dem Konzept von MVCC: Statt ausschließlich jene Objektversionen zu berücksichtigen, die zum Beginn der Transaktion aktuell waren, wird beim ersten Zugriff auf ein Objekt immer die neueste Version aus der Datenbasis gelesen.\footnote{Dies entspricht dem Konzept der chronologieerhaltenden Serialisierbarkeit\cite{DBLP:books/sp/HarderR01}.} Alle nachfolgenden Lesezugriffen beziehen sich auf diese Objektversion. Alle Mehrbenutzeranomalien werden vermieden, dies entspricht der Isolationsebene \texttt{SERIALIZABLE}.\footnote{Zu beachten ist, dass dies ausschließlich für die eingebettete Verwendung gilt. Im Client-Server-Betrieb werden Lesezugriffe immer vom Server beantwortet, folglich können die Anomalien Non-Repeatable Read und Phantom Problem auftreten. Dies entspricht der Isolationsebene \texttt{READ COMMITTED}\cite{Orient_mvcc_problem:2013}.}

Kommt es vor dem Commit einer Transaktion zum Ausfall des GDBMS oder des Clients, so sind die bis zu diesem Zeitpunkt erfolgten Änderungen nicht in der Datenbasis vorhanden, folglich ist in den genannten Situationen die atomare Ausführung der Transaktion sichergestellt. Kommt es hingegen beim Ausführen des Commit zum Ausfall des GDBMS, können bereits Teile einer Transaktion in der Datenbasis vorhanden sein. Um diese Änderungen widerrufen zu können, wird vor dem Ausführen jeder Änderung ein zugehöriger Eintrag in ein Transaktions-Log geschrieben. Beim Löschen oder Ändern eines Datensatzes enthält der Log-Eintrag eine Kopie des ursprünglichen Datensatzes.\footnote{Dies entspricht dem Konzept des physischen Loggings\cite{DBLP:books/sp/HarderR01} und kann für das Aktualisieren im Quelltext unter \url{https://github.com/orientechnologies/orientdb/blob/1.5.1/core/src/main/java/com/orientechnologies/orient/core/storage/impl/local/OStorageLocalTxExecuter.java\#L115} nachvollzogen werden.} Das Ausschreiben der Log-Daten auf den Externspeicher erfolgt unmittelbar, beim Neustart des GDBMS kann somit ein konsistenter Zustand wiederhergestellt werden.  Aus den Ausführungen geht hervor, dass es sich um ein UNDO-Log handelt.\\
 OrientDB nutzt Memory-Mapped-Files und überlässt es folglich dem Betriebssystem, wann ausgeschriebene Änderungen tatsächlich auf den Externspeicher geschrieben werden. Das Transaktions-Log enthält jedoch keine REDO-Informationen erfolgreich beendeter Transaktionen. Folglich sind deren Änderungen verloren, wenn diese sich zum Zeitpunkt eines Systemausfalls noch nicht auf dem Externspeicher befinden. Das GDBMS ermöglicht das Aktivieren einer FORCE-Strategie, bei der sowohl Log-Informationen als auch Änderungen an der Datenbasis unmittelbar auf den Externspeicher geschrieben werden. Dies ist jedoch mit entsprechenden Performance-Einbußen verbunden.
...
- für eingebettete Verwendung
- Level 1 Database Level (1 Cache per Thread)
- Weak References LinkedHashMap
- Level 2 Storage Level (1 per JVM)
- LinkedHashMap

- Aktualisierung erfolgt nach erfolgreichem Commit, nachzuvollziehen in \url{https://github.com/orientechnologies/orientdb/blob/1.5.1/core/src/main/java/com/orientechnologies/orient/core/storage/impl/local/OStorageLocalTxExecuter.java#L189}
	- zuerst Level 1 Cache
	- wenn CLuster erlaubt, dann Level 2 Cache
