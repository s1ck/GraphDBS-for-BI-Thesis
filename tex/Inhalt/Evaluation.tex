...
Das Transaktions-Log wird permanent im Hauptspeicher gehalten. Kommt es während der Ausführung einer Transaktion zum Ausfall des GDBMS, sind die bis dahin erfolgten Änderungen ausschließlich im Hauptspeicher und somit verloren. Die Atomarität einer Transaktion ist damit sichergestellt und eine UNDO-Recovery nicht erforderlich. Für die Wiederholbarkeit erfolgreicher Transaktionen wird beim Commit  ein entsprechender Eintrag in das Log geschrieben. In der Standardeinstellung von BerkeleyDB führt dieser Eintrag zum Ausschreiben des Logs auf den Hintergrundspeicher, eine REDO-Recovery ist folglich möglich. HyperGraphDB initialisiert BerkeleyDB standardmäßig mit einer Konfiguration, die es der Datenbank erlaubt, das Transaktions-Log asynchron auf den Hintergrundspeicher zu schreiben, was bedeutet, dass Änderungen zum Commit-Zeitpunkt nicht unmittelbar persistiert werden.\footnote{Die Initialisierung kann im Quelltext unter \url{https://code.google.com/p/hypergraphdb/source/browse/trunk/storage/bdb-je/src/java/org/hypergraphdb/storage/bje/BJEConfig.java} in Zeile 64 nachvollzogen werden} Kommt es zwischen Commit und Persistieren des Logs zum Systemausfall, sind die Änderungen der Transaktion verloren.

\subsection{Persistenz-, Cache- und Indexverwaltung}

Wie anfangs erwähnt, ist HyperGraphDB ein nicht-natives GDBMS, welches den Graphen auf assoziative Arrays abbildet und diese für die Anfrageausführung sowie die physische Repräsentation der Datenbasis verwendet. Die Organisation erfolgt dabei innerhalb des GDBMS in zwei Schichten: Der primitiven Speicherschicht, welche direkt auf das eingesetzte Speichersystem zugreift, und der Modellschicht, welche von der primitiven Schicht abstrahiert und deren Informationen als Elemente des Datenmodells repräsentiert. Das eingesetzte Speichersystem wird nachfolgend als physische Speicherschicht bezeichnet, da es letztendlich die Persistenz der Daten sicherstellt, standardmäßig ist dies BerkeleyDB.

Die primitive Speicherschicht bildet die Topologie des Graphen und die Rohdaten der Atomwerte ab, Typen werden auf dieser Ebene noch nicht berücksichtigt. Für die Speicherung werden zwei assoziative Arrays verwendet:

\texttt{LinkStore: ID $\rightarrow$ List<ID>}\newline
\texttt{DataStore: ID $\rightarrow$ List<byte>}

Der \texttt{LinkStore} bildet die Struktur des Graphen ab, indem er Identitäten (IDs) eine Liste weiterer Identitäten zuordnet. Im \texttt{DataStore} werden jeder Identität Rohdaten in Form eines Byte-Arrays zugeordnet. Jeder Datensatz in der primitiven Speicherschicht ist folglich ein Schlüssel-Wert-Paar. Der Schlüssel wird in HyperGraphDB durch ein spezielles Handle repräsentiert, das sog. \texttt{HGPersistentHandle}. Dieses ordnet jedem Atom eine UUID fester Länge zu. Innerhalb der primitiven Speicherschicht ist somit der \texttt{LinkStore} eine Liste von Instanzen des genannten Handles.\\
In der physischen Speicherschicht werden die Werte beider Arrays als Byte-Arrays abgelegt. Die Interpretation der Werte im \texttt{LinkStore} erfolgt in der primitiven Speicherschicht, ermöglicht wird dies durch die feste Länge der UUIDs. Die Werte im \texttt{DataStore} können auf dieser Ebene noch nicht interpretiert werden, da keine Informationen zum Datentyp vorhanden sind. Es sollte deutlich werden, dass die Schlüsselmengen im \texttt{LinkStore} und \texttt{DataStore} identisch sind und somit zwei getrennte Key-Value-Datenbanken notwendig sind um die Informationen zu speichern.

Die Modellschicht wandelt die Informationen der primitiven Speicherschicht in die abstrakten Elemente des Datenmodells um. Nachfolgend sind alle ID-Bezeichner als Instanzen von \texttt{HGPersistentHandle} zu verstehen. Ein Schlüssel-Wert-Paar im \texttt{LinkStore} wird in der Modellschicht als Atom interpretiert, jeder Position innerhalb des Wertes ist eine feste Semantik zugeordnet:

\texttt{AtomID $\rightarrow$ [TypeID, ValueID, TargetID, ..., TargetID]}

Die \texttt{TypeID} verweist auf den Typ des Atoms während die \texttt{ValueID} den zugehörigen Wert referenziert. Anschließend wird die Zielmenge des Atoms aus einer Menge von \texttt{TargetIDs} definiert. Da ein Knoten im Datenmodell ein Atom mit Arität Null ist, enthält seine physische Repräsentation im Gegensatz zu einer Kante nur den Verweis auf seinen Typ und den Wert. Da sowohl der Typ eines Atoms als auch die Einträge in der Zielmenge Atome sind, verweisen \texttt{TypeID} und \texttt{TargetID} rekursiv auf eine \texttt{AtomID}.

Der Wert eines Atoms ist im \texttt{DataStore} der primitiven Datenschicht hinterlegt. In der Modellschicht wird die \texttt{ValueID} eines Atoms wie folgt abgebildet:

\texttt{ValueID $\rightarrow$ List<ID> | List<byte>}

Für primitive Datentypen wird direkt das Byte-Array angegeben, während sich komplexe Datentypen aus mehreren Einträgen im \texttt{DataStore} zusammensetzen. An dieser Stelle wird auch anhand der Speicherung deutlich, warum Werte an Atomen grundsätzlich unveränderlich sind und eine Wertänderung nur durch den Austausch des Wertes erfolgen kann.\\
Die Interpretation der Werte erfolgt nicht in der Modellschicht, sondern wird von der jeweiligen Typimplementierung bzw. vom jeweiligen Typkonstruktur durchgeführt. Im Zusammenhang mit dem Typsystem wurde bereits erwähnt, dass Typen abstrakte Fabriken sind und unter anderem eine Methode zum Erzeugen der entsprechenden Instanzen zur Verfügung stellen. Diese Methode nimmt die Rohdaten entgegen und führt die Deserialisierung durch.
