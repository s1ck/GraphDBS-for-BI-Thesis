...
Die Deserialisierung der Werte erfolgt nicht in der Modellschicht, sondern wird von der jeweiligen Typimplementierung bzw. vom jeweiligen Typkonstruktur durchgeführt. Im Zusammenhang mit dem Typsystem wurde bereits erwähnt, dass Typen abstrakte Fabriken sind und unter anderem eine Methode zum Erzeugen der entsprechenden Instanzen zur Verfügung stellen. Diese Methode nimmt das persistente Handle entgegen und führt die Deserialisierung durch.
...
Der \texttt{IncidenceIndex} ordnet jedem Atom seine Inzidenzmenge zu, d.h. die Menge aller Kanten die auf das Atom zeigen. Der \texttt{TypeIndex} verweist auf die Menge aller Instanzen eines gegebenen Datentyps und der \texttt{ValueIndex} weist einem Wert die Menge der Atome zu, die ihn besitzen. Bei verschachtelten Werten wird der Wert auf höchster Ebene indexiert. Zu beachten ist, dass die zugeordneten Werte Tupel sind und somit über eine Ordnung verfügen.\\
Mit Hilfe der Indexstrukturen lassen sich graphenspezifische Operationen effizient ausführen. Zum Beispiel lässt sich anhand der Schnittmenge der Inzidenzlisten zweier Knoten prüfen, ob diese adjazent sind.
