...
HyperGraphDB ist ein quelloffenes, disk-zentriertes GDBMS, welches von Kobrix Software\footnote{\url{http://kobrix.com/index.jsp}} entwickelt wird. Die Quellcode-Lizenz ist LGPL, die erste stabile Version 1.0 wurde 2010 veröffentlicht. Die Implementierung des Systems erfolgt ausschließlich in Java und sieht den Einsatz als eingebettetes Graphdatenbanksystem vor. Die Verarbeitung des Graphen erfolgt nativ, für die Datenspeicherung wird hingegen standardmäßig die quelloffene Key-Value-Datenbank BerkeleyDB Java Edition\cite{Oracle_BerkeleyDB:2013} eingesetzt. HyperGraphDB kann als zentrales System oder in einer verteilten Konfiguration eingesetzt werden. Eine rein hauptspeicher-zentrierte Verwendung ist nicht vorgesehen.

Grundlegend betrachtet handelt es sich bei HyperGraphDB um ein objekt-orientiertes Persistenz-Framework zur Speicherung von Java-Objekten und kann auch als solches eingesetzt werden. Die Repräsentation im Hauptspeicher, Indexierung und Caching sind jedoch für die Unterstützung von Traversierung und Mustersuche konzipiert. Wesentliches Merkmal und gleichzeitig Alleinstellungsmerkmal von HyperGraphDB ist die Möglichkeit zur Modellierung von n-ären Beziehungen, als Datenmodell wird das Property-Hypergraph-Modell in Verbindung mit einem erweiterbaren Typsystem implementiert. Konzipiert wurde das GDBMS für die Verwaltung hochkomplexer Daten wie sie zum Beispiel in der Wissensverwaltung, künstlichen Intelligenz und im Semantic Web vorkommen\cite{Iordanov:2010:HGG:1927585.1927589}. Laut Hersteller eignet sich das System darüber hinaus auch für den Einsatz in sozialen Netzwerken sowie in regulären objekt-orientierten Geschäftsanwendungen\cite{HGDB_business_applications:2012}. Ein weiteres Merkmal ist die flexible Architektur des Systems, welche die Implementierung und Integration eigener Indizierungs-, Persistenz- und Verteilungsmechanismen ermöglicht.
...
% Modelltheorie
Die Basiseinheit innerhalb des Modells wird in HyperGraphDB als Atom bezeichnet. Sowohl Knoten als auch Kanten werden durch Atome repräsentiert: Jedes Atom besitzt eine sog. Zielmenge, welche sich durch ein Tupel bestehend aus Atomen definiert. Die Kardinalität dieses Tupels wird als Arität bezeichnet, ein Atom mit Arität Null ist ein Knoten andernfalls eine Kante.\footnote{Die Begriffe Kante und Hyperkante werden bei der Beschreibung von HyperGraphDB synonym verwendet.}\\
Eine Kante kann folglich auf eine beliebige Anzahl von Atomen verweisen. Dies stellt eine Erweiterung des in Abschnitt \ref{subsec:hgm} definierten Modells eines Hypergraphen dar, da eine Kante in HyperGraphDB sowohl auf Knoten, als auch auf andere Kanten verweisen kann. Als Inzidenzmenge eines Atoms $a$ wird die Menge der Atome bezeichnet die $a$ in ihrer Zielmenge enthalten, d.h. alle Kanten, deren Zielknoten $a$ ist.

Neben der Zielmenge besitzt jedes Atom einen streng typisierten Wert. Der Typ kann dabei beliebig sein und wird innerhalb des Modells ebenfalls als Atom repräsentiert. Die Semantik wird folglich zusammen mit den Daten modelliert, gleichzeitig erfolgt eine Trennung von Topologie und Nutzdaten innerhalb des Graphen: Solange sie einem definierten Typ entsprechen, können Nutzdaten an Atomen ohne Änderung der Topologie beliebig ausgetauscht werden. Eine dynamische Schemaänderung ist somit möglich.

Das Typsystem in HyperGraphDB erlaubt

% Typisierung

- ermöglicht die Interpretation der Daten
	- Integrität und Konsistenz
	- anpassbares Speichermodell
	- erlaubt dynamische Schemaänderung
- Typen sind ebenfalls Atome -> Schema ist Teil der Daten
- Typkonstruktoren erlauben das definieren beliebiger Typsysteme
- Definition von Meta-Data und Reasoning (z.B. der Typ Kauf ist das Gegenteil vom Sale)
- HyperGraphDB erlaubt das standardmäßig das direkte Speichern folgender Typen
	- primitive Java-Datentypen sowie deren Arrays (byte buffers)
	- Java-Bean-Klassen
	- Arrays und Collections von Objekten
	- Maps als Sequenz von Objektpaaren
	- speichert Vererbungsinformationen
- weitere Typen:	
	- vordefinierte Typen (z.B. Kanten ohne Properties)
- eigene Typen:
	- ermöglicht Einbettung von Objekten

% Modellimplementierung

- Knoten-Atome sind beliebige Java-Objekte
- Kanten-Atome implementieren spezielle Schnittstelle \texttt{HGLink}
- Verwaltung eines Atoms via Handle (Instanz eines Handle ist die Identität eines Atoms)
	- keine Primärschlüssel, Identität ist das Objekt
	- Handle ist einer Markierungsschnittstelle
	- Handle ist vergleichbar mit Objektreferenz in Java bzw. Zeiger in C/C++
	(- Handle-basierte Verarbeitung wird empfohlen, abrufen des entsprechenden Java-Objektes nur bei Bedarf)

- jedes Atom besitzt zwei Bezeichner
	- erster Bezeichner definiert den Typ des Atoms
	- zweiter Bezeichner speichert den Wert (entsprechend dem Typ)
- Typen sind ebenfalls Atome -> ermöglicht Abbildung von Vererbungshierarchien

- Gehrels\cite{Gehrels:2013}
	- Kanten und Knoten sind Atome
	- Kanten können auf andere Kanten zeigen
	- Kanten können von anderen Kanten ausgehen
	- Kanten können von mehreren Knoten und Kanten ausgehen
	- Kanten können auf mehrere Knoten und Kanten zeigen
	
- Implementierung ermöglicht das Verwalten mehrerer Datenbanken
	- sind jedoch unabhängig voneinander

- Orientierung näher an objekt-orientierten Datenbanken wegen Generalität / Allgemeingültigkeit
	- Einbettung von Objekten
	- n-äre Beziehungen
	- Typisierung von Relationen ermöglicht Reifizierung (z.B. in RDF ein Tripel bei dem das Subjekt ein Tripel ist = Aussage über Aussagen)
	- sehr flexibles, folglich mächtiges, vielseitiges Datenmodell
