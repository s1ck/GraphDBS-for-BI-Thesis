...
}} entwickelt wird. Der Quellcode steht unter Apache 2.0 Lizenz, was bedeutet, dass er in freien, aber auch auch in kommerziellen Anwendungen verwendet werden darf. Das GDBMS wurde 2010 veröffentlicht, die erste stabile Version 1.0 erschien 2012. Für die Implementierung wird ausschließlich Java verwendet, die Ausführung ist somit analog zu Neo4j und HyperGraphDB an die JVM gebunden und auf kompatiblen Plattformen möglich. OrientDB ist ein natives GDBMS, sowohl Modellierung und Zugriff als auch Verarbeitung und Speicherung orientieren sich an der Topologie der Daten. Das DBMS wird entweder in Form einer eingebetteten Bibliothek innerhalb von Java-Anwendungen oder in einer Client-Server-Konfiguration verwendet. Der Server bietet den Zugang über REST-Schnittstellen an, Clients stehen in vielen Programmiersprachen zur Verfügung.\footnote{\url{https://github.com/orientechnologies/orientdb/wiki/Programming-Language-Bindings}} OrientDB kann sowohl als zentrales GDBMS als auch in einer verteilten Konfiguration eingesetzt werden. Hierfür wird eine Master-Master-Replikation realisiert, welche die horizontale Skalierbarkeit von Lese- und Schreibanfragen sowie eine erhöhte Ausfallsicherheit gestattet. Die Speicherung der Datenbasis erfolgt disk-zentriert, das System bietet zwar die ausschließliche Nutzung des Hauptspeichers an, dies entspricht jedoch nicht der definierten hauptspeicher-zentrierten Speicherung, sondern ist für die Verwendung im Rahmen von Unit-Tests konzipiert.

% Besonderheiten (Dokumente, Typsystem) + Anwendungsfall
Eine Besonderheit von OrientDB gegenüber den vorgestellten Systemen ist die Möglichkeit zur Verwendung verschiedener Datenmodelle: Die Speicherung der Information erfolgt grundlegend in Form von Dokumenten, welche aus Schlüssel-Wert-Paaren bestehen und beliebig ineinander verschachtelt sein können. Ein darauf aufbauendes graphenorientiertes Datenmodell ermöglicht die Definition von Beziehungen zwischen Dokumenten. Darüber hinaus bietet OrientDB den Einsatz als objektorientierte Datenbank an, wobei Objekte ebenfalls auf Dokumente abgebildet werden. Das DBMS erlaubt im Dokumenten-, Graph- und auch im objektorientierten Datenmodell die Definition eines Schemas zur Modellierung einer Anwendungsdomäne und zur Festlegung von Integritätsbedingungen. Der Verzicht auf ein Schema und die Kombination aus strukturierten und semi-strukturierten Daten ist hingegen ebenfalls möglich.\\
Ein Alleinstellungsmerkmal von OrientDB in der vorliegenden Evaluation ist die Verwendung des SQL-Standards für die Formulierung von Anfragen an die Datenbasis. OrientDB stellt eine Teilmenge des Standards zur Verfügung und erweitert diese mit graphenspezfischen Operationen. Ein weiteres Alleinstellungsmerkmal ist die bereits im DBMS integrierte Nutzer- und Rollenverwaltung.

% Ausführungen beziehen sich auf ... + Quellen
Die nachfolgende Evaluation basiert auf der im September 2013 freigegebenen Version 1.5.1. Der Großteil der Informationen stammt aus den Ausführungen zu OrientDB in \cite{EdlichFriedlandHampeBrauer201010} und der offiziellen Dokumentation\cite{Orient_doku:2013}. Es wird ausschließlich das Dokumenten- und das darauf aufbauende Graphdatenmodell berücksichtigt, da vor allem letzteres für die Evaluation relevant ist.
...
% Dokumentenmodell
- Dokumentenmodell ist die Grundlage aller weiteren Datenmodelle

- grundlegende Einheit ist das Dokument
	- Menge von Schlüssel-Wert-Paaren
	- eindeutige Schlüssel
	- native Unterstützung vordefinierter Typen
		- primitive Java-Datentypen
		- Listen, Mengen, assoziative Arrays
	- Schachtelung von Objekten ist Möglich (Objekteinbettung)
	- systemseitige Versionierung
	- Nutzung erfolgt wahlweise mit oder ohne Schema bzw. gemischt
	- Schema = Typ
	- schwache Typisierung: Schema kann existieren, jedoch mit zusätzlichen Attributen erweitert werden


% Graphenmodell

- Property-Graph-Modell
	- setzt auf Dokumentenmodell auf
	- PGM ist grundsätzlich schemalos
	- Unterschied: Daten können komplett oder nur teilweise einem Schema unterliegen bzw. komplett auf ein solches verzichten
	- Schema erlaubt Attributeinschränkungen, Definition von Indizes und obligatorischen Attributen


% Knoten: Label?, Identität, Schema
- repräsentiert durch Dokument
% Kanten: Label?, Richtung, Identität, Schema
- repräsentiert durch Dokument
- zwei Arten von Beziehungen:
	- referenziert
		- werden explizit gespeichert (Dokument) -> eigene Identität
		- Richtung wird gespeichert
		- Typen
			- 1-1 / N-1 -> \texttt{LINK}
			- 1-N / N-M -> \texttt{LINKLIST} (ordered), \texttt{LINKSET} (unordered, no duplicates) \texttt{LINKMAP} (sortierte Abbildung von Kanten mit String Bezeichnern, no duplicates)
	- eingebettet
		- werden in das Dokument eingebettet (Komposition) -> keine eigene Identität
		- Kantenbezeichner ist ein Attribut am Startknoten, Wert ist der Zielknoten
		- Typen
			- 1-1 / N-1 -> \texttt{EMBEDDED}
			- 1-N / N-M -> \texttt{EMBEDDEDLIST}, \texttt{EMBEDDEDSET}, \texttt{EMBEDDEDSET},
	- Kanten lassen sich NICHT in beiden Richtungen traversieren
	- wird eine Kante in umgekehrter Richtung angelegt, muss sich der Entwickler um die Einhaltung der Konsistenz kümmern -> modellinhärente Integrität
- Bezeichner wird durch die Klasse der Kante festgelegt
	
% Datenbank
- mehrere Instanzen möglich
- eindeutiger Bezeichner
- verschiedene Datenbanktypen je nach erforderlichem Datenmodell
- verschiedene Datenbankengines je nach Verwendung
	- lokal
	- entfernt
	- in-memory (z.B. für Unit-Test)

	
% Properties: JSON-Dokument?, zulässige Werte, Null-Werte?
% Datenintegrität: modellinhärent, attributbezogene Integritätsbedingungen, Wertebereiche, Primärschlüssel, Unique
...
- Definition von Klassen
	- besitzen Attribute
		- vordefinierte Typen für Attribute \url{https://github.com/orientechnologies/orientdb/wiki/Types}
		- Attribute können Klassen sein -> Dokument-Konzept (Einbettung)
		- Einschränkungen
			- explizite Wertebereichseinschränkungen (min, max)
			- UNIQUE Index -> Primärschlüssel
			- Mandatory
			- keine Updates (nur initiales Schreiben)
			- not null
			- reguläre Ausdrücke
			
	- legt den Typ von Records fest
	- Klassen können schemalos, schema-full oder gemischt sein
	- können abstrakt sein
	- 

- Vererbung wird erlaubt
	- Attribute werden vererbt	
	- alles public
	- Anfragen sind polymorph (betreffen auch Subtypen)
	- keine Mehrfachvererbung	
	- oberste Klasse: \texttt{OGraphVertex}
	- außer oberste Klasse muss die Vererbung explizit angegeben werden
...
- vergleichbar mit relationalen Datenbanksystemen (z.B. MySQL)
- Definition von Nutzern und Rollen
	- Nutzer besitzt mehrere Rollen
		- Rolle definiert sich durch eine Menge von Regeln
		- Regeln werden entweder erlaubt oder verboten (Whitelist- und Blacklist-Prinzip)
	- Nutzer identifiziert sich via Passwort
	- es existiert ein Nutzer \texttt{root} zur Verwaltung von Nutzern und Rollen
- Record Level Security

\subsection{Zugriffsmechanismen}

- native Java API für Dokumentenmodell
- native Java API für Graphenmodell
- native Java API für Objektmodell
- Traversierung via Gremlin (-> Titan)
- SQL
- Besonderheiten

\paragraph*{Blueprints Java API}

\paragraph*{SQL}

\paragraph*{Traversierung in SQL}
