...
Die wesentliche Aufgabe eines Typen ist die Serialisierung von Laufzeitobjekten in das Format der darunterliegenden Speicherschicht sowie die Deserialisierung aus der Speicherschicht in Laufzeitobjekte. Im Sinne der Objektorientierung ist ein Typ somit eine abstrakte Fabrik und stellt Methoden zum Erzeugen, Speichern und Löschen von Laufzeitrepräsentationen seiner Instanzen zur Verfügung.\footnote{Das Aktualisieren eines Wertes ist nicht möglich, da Werte in HyperGraphDB generell unveränderlich (engl. \textit{immutable}) sind. Dies hat den Vorteil, dass ein Wert mehreren Atomen zugeordnet sein kann, führt jedoch dazu, dass bei der Aktualisierung eines Wertes ein neuer Wert erzeugt werden muss.} Da es sich bei Typen um Atome handelt, besitzen auch Typen einen übergeordneten Datentyp. Bei Typen, die bereits im Typsystem existieren, ist dies ein spezieller Typ mit der Bezeichnung \texttt{top}. Wird hingegen ein Objekt, dessen Typ nicht bereits in der Datenbasis hinterlegt ist, zur Laufzeit eingefügt, so ist dessen Typ ein Typkonstruktur. Ein Typkonstruktur erzeugt neue Typen aus bestehenden Typen, es handelt sich folglich um einen Datentyp, dessen Instanzen Typen sind.\\
...
Es sollte deutlich werden, dass ein Typ in HyperGraphDB ein Atom mit einer speziellen Rolle ist. Aus Anwendungssicht ermöglicht es die Definition von Knoten- und Kantenbezeichnern, aus Systemsicht die Verwaltung von Assoziationen sowohl zwischen einem Typ und seinen Instanzen als auch zwischen mehreren Typen. Das GDBMS beinhaltet vordefinierte Typen für primitive Java-Datentypen, JavaBeans\footnote{\url{http://www.oracle.com/technetwork/java/javase/documentation/spec-136004.html}}, 
...
HyperGraphDB bietet keine eigene Anfragesprache an, sondern erlaubt den Zugriff auf die Datenbasis ausschließlich via Java API im eingebetteten Betrieb. Dabei stehen Operationen zum Erzeugen und Löschen von Atomen, mengenorientierte Operationen und die Möglichkeit zur Traversierung des Graphen zur Verfügung.
...
%Knoten
Ein Knoten definiert sich durch eine Objektinstanz innerhalb der Laufzeitumgebung. Diese lässt sich dem Graphen hinzufügen: Ist ihr Datentyp bereits im Typsystem registriert, wird er automatisch mit dem Knoten verbunden, andernfalls zunächst durch einen Typkonstruktor erzeugt. Das Resultat des Anlegens ist ein Handle, welches den Knoten innerhalb der Datenbasis identifiziert. Anhand des Handles kann die Objektinstanz und deren Typ aus der Datenbasis gelesen werden, umgekehrt lässt sich unter Angabe der Objektinstanz das zugehörige Handle laden. Das Löschen eines Atoms ist ausschließlich unter Angabe des zugehörigen Handles möglich. In Verbindung mit dem flexiblen Typsystem und der vordefinierten Typen lassen sich somit beliebige Nutzdaten an einem Knoten speichern. Wird zum Beispiel eine JavaBean dem Graphen hinzugefügt, so wird diese intern durch eine Record-Struktur repräsentiert, die den schlüsselbasierten Zugriff auf die Attribute des Objektes ermöglicht. Da diese Attribute wiederum typisierte Werte sind, lassen sich beliebig verschachtelte Objekte hinterlegen.

%Kanten
Kanten werden in HyperGraphDB nicht durch Objektreferenzen zwischen Objektinstanzen, sondern durch spezielle Kantenobjekte repräsentiert. Letztere sind Implementierungen der Schnittstelle \texttt{HGLink}. Das GDBMS stellt drei verschiedene Implementierungen zur Verfügung: Kanten können ohne jegliche Zusatzinformationen, zum Beispiel einer semantischen Bedeutung oder Nutzdaten, angelegt werden. Eine zweite Implementierung erlaubt das Speichern von Nutzdaten an der Kante, dies kann eine beliebig komplexe Objektinstanz oder eine einfache Zeichenkette zur Festlegung eines Kantenbezeichners sein. Ein dritter Typ ermöglicht zudem das Festlegen zulässiger Typen innerhalb der Zielmenge und gestattet damit die Definition von Integritätsbedingungen.\\
Kanten werden unter Angabe ihrer Zielmenge erzeugt und durch ein Handle identifiziert. Das Auslesen und Löschen erfolgt analog zu Knoten. Kanten stellen darüber hinaus Funktionen zum Auslesen der Zielmenge und der Arität zur Verfügung. Da die Zielmenge ein Tupel und somit geordnet ist, können Handles unter Angabe ihrer Position innerhalb der Zielmenge ausgelesen werden. Dies ermöglicht zum Beispiel die Definition von Kantentypen die einzelnen Positionen innerhalb der Zielmenge eine Semantik zuordnen.

\paragraph*{Mengenorientierte Anfragen}

- API ermöglicht Formulierung und Verknüpfung von Anfragebedingungen (\texttt{HQQueryCondition})
 	- werden an HGDB gesendet
 	- Menge von Atomen als Ergebnis
- zwei Arten von Ausdrücken für Atome:
	- Bezug auf den Wert am Atom (\texttt{AtomTypeCondition}, \texttt{AtomValueCondition})
	- Bezug auf Topologie des Graphen (\texttt{AtomPartCondition})

- konjunktive und disjunktive Verknüpfung von Prädikaten
	- Prädikate beziehen sich entweder auf den Wert des Atoms oder auf die Topologie des Graphen
	- beliebige Schachtelung
	- Ergebnis ist eine Menge von Handles
	- grundlegende Prädikate:
		- eq(x), eq("name", x), lte, gt, gte für den Wert eines Atoms
		- type(typeID) - Prüfen des Wertes
	- Beispiel:
		
Wert:	

\texttt{hg.and(hg.type(Product.class), hg.gte(\string"salesrank\string", 2))}

Topologie:

\texttt{hg.and(hg.incident(handleInstance), hg.arity(10)}

Topologie und Wert:

\texttt{hg.or(hg.subsumes(Person.class), hg.eq(\string"age\string", 25))}


\paragraph*{Traversierung}

	- Breiten- und Tiefensuche
	- Adjazenlistengenerator
