...
\begin{table}[h]
	...
	\textbf{\texttt{sim\_products}} & Selektion aller ähnlichen Produkte deren Abstand höchstens Zwei beträgt. Jedes Produkt soll einmalig in der Ergebnismenge sein. \\
	...
	\textbf{\texttt{path\_all}} & Berechnen aller Pfade der Länge Vier zwischen einem Nutzer und einem Produkt. \\

	\textbf{\texttt{path\_shortest}} & Berechnung des kürzesten Pfades zwischen zwei Nutzern unter Verwendung der Beziehungstypen \texttt{FRIEND\_OF}, \texttt{REVIEWED\_BY} und \texttt{SIMILAR\_TO}. Die maximale Pfadlänge beträgt Fünf. \\

	\textbf{\texttt{top\_regions}} & Berechnen der Anzahl Bücher mit Salesrank > 1000 gruppiert nach Region. \\

	\textbf{\texttt{triangle\_pattern}} & Finden aller Dreicksbeziehungen folgender Art: Ein Produkt \texttt{p} besitzt eine Bewertung \texttt{r} die für mindestens 10 Personen hilfreich war. Der bewertende Nutzer \texttt{u1} ist mit einem weiteren Nutzer \texttt{u2} befreundet, der älter als 20 Jahre alt ist und ebenfalls das Produkt p bewertet hat. Es sollen \texttt{p}, \texttt{u1}, \texttt{u2} und \texttt{r} bestimmt werden. \\
