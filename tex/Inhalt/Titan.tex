...
% Zugriff auf Knoten
- Menge aller Knoten \texttt{g.V}
- Menge aller Kanten \texttt{g.E}

	% Index name='Alice'
- Zugriff auf einzelne Knoten anhand ihrer Identität oder via Index	
\texttt{alice = g.v(name='Alice')}
- Zugriff auf Attributwerte durch Angabe der Attributschlüssel
\texttt{alice.age}

% Beispiele
	% Einschränkung Kantenbezeichner und Richtung (Whitelist)
\texttt{alice.out('WORKS\_WITH').name}
\texttt{alice.outE.label}

- \texttt{in} und \texttt{both}
...
- via Filter-Funktion
	...
	% Beispiel Alter > 30
\texttt{alice.outE.filter\{it.since > 2011\}.inV.filter\{it.age > 30\}}
