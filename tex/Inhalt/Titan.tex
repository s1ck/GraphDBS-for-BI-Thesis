...
Lese- und Schreibzugriffe erfolgen in Titan grundsätzlich im Kontext einer Transaktion. Diese wird beim ersten Zugriff automatisch gestartet und ist an den erzeugenden Thread gebunden. Alle nachfolgenden Operationen nutzen die aktive Transaktion bis diese explizit beendet wird. Hierfür stehen die Funktionen Commit und Rollback zur Verfügung. Kommt es während der Transaktionsausführung zu einem Fehler, wird dieser an die Anwendung weitergegeben. Titan unterscheidet zwischen temporären und dauerhaften Fehlern: Ein temporärer Fehler ist zum Beispiel der kurzzeitige Verbindungsabbruch im Client-Server-Betrieb. Das GDBMS bietet die Option, entsprechend abgebrochene Transaktionen automatisch zu wiederholen. Als dauerhafte Fehler klassifiziert Titan zum Beispiel System- oder Hardwarefehler, bei denen das GDBMS gestoppt wurde. 
...
Titan bildet den Graphen auf ein Datenmodell ab, das nicht für die native Verarbeitung vernetzter Daten konzipiert ist. Es handelt sich um das Key-Column-Value-Modell (KCV), welches von Google entwickelt wurde und im Datenbanksystem BigTable Verwendung findet\cite{Chang:2006:BDS:1267308.1267323}. Es wird u.a. von Apache Cassandra und Apache HBase implementiert und erweitert das Key-Value-Modell um eine zusätzliche Dimension:  Jedem Key ist eine Menge von Einträgen zugeordnet, wobei jeder Eintrag aus einem eigenen Bezeichner (Column) und einem zugeordneten Wert (Value) besteht. Das Modell ist somit vergleichbar mit einer Tabelle im relationalen Datenmodell, wobei jede Zeile individuell verschiedene Attribute besitzen kann.\\
Eine Beschreibung der Persistenzverwaltung ist bisher nicht Teil der offiziellen Dokumentation des GDBMS\footnote{Laut Mailing-Liste ist eine entsprechende Dokumentation für Version 0.4.0 des GDBMS geplant: \url{https://groups.google.com/forum/?hl=de\#!topic/aureliusgraphs/ixPXcG4UvSQ}.}, ein vollständiges Reverse Engineering wäre jedoch Thema für eine eigenständige Abschlussarbeit. Die nachfolgenden, vereinfachten Erläuterungen basieren auf den Resultaten eines Reverse Engineering\cite{github_delft:2013} vom Juni 2013 und einer teilweisen Untersuchung des Quelltextes. Die Speicherung in Titan lässt sich in zwei Schichten unterteilen: Eine Speicherschicht, welche den Graphen auf das KCV-Modell abbildet und eine Datenbankschicht, welche die hinterlegten Informationen in das Property-Graph-Modell überführt und für die Anwendung zur Verfügung stellt. Nachfolgend wird die Abbildung innerhalb der Speicherschicht erläutert. 

Titan verwendet insgesamt vier KCV-Datenbanken für die Speicherung der Datenbasis und zugehöriger Indexstrukturen. Der \texttt{EdgeStore} beinhaltet die Topologie des Graphen sowie die an Knoten und Kanten hinterlegten Nutzdaten. Analog zu OrientDB und HyperGraphDB werden diese in Titan nicht getrennt gespeichert. Abbildung \ref{fig:titan_edgestore_row} zeigt die vereinfachte Darstellung eines Knotens im KCV-Modell am Beispiel des Mitarbeiters Alice.

\begin{figure}[h] 
	\centering
		\includegraphics[scale=.8]{titan_edgestore_record.pdf}
	\caption[Titan: Datensatz eines Knotens]{Vereinfachte Darstellung der Repräsentation eines Knotens innerhalb der KCV-Datenbank. Grau hinterlegte Bereiche dienen lediglich der Beschreibung der einzelnen Komponenten einer Zeile innerhalb des KCV-Modells.}
	\label{fig:titan_edgestore_row}
\end{figure}

Die Speicherung erfolgt knoten-zentriert: Unter Angabe einer Knoten-Identität kann auf den zugehörigen Datensatz zugegriffen werden. Ein Eintrag innerhalb des Datensatzes repräsentiert entweder die Werte eines Knotenattributes oder die Kanteninstanzen zu einem Kantenbezeichner. Die Unterscheidung erfolgt anhand der Column eines Eintrages: Diese ist entweder ein \texttt{TitanKey} oder ein \texttt{TitanLabel}. In der Abbildung wird der Typ durch seinen global eindeutigen Namen dargestellt (z.B. \texttt{age}).\\
Handelt es sich um einen als \texttt{UNIQUE} deklarierten Typ, so ist dem Eintrag genau ein Wert zugeordnet. Dies trifft im Beispiel auf alle Attributschlüssel des Knotens zu. Im Gegensatz dazu werden die Werte von gewöhnlichen Typen in sog. Super-Columns zusammengefasst. Die Abbildung verdeutlicht dies am Beispiel der \texttt{WORKS\_WITH}-Beziehung: Alle Kanteninstanzen mit dem entsprechenden Bezeichner befinden sich in einer Super-Column. Jede Instanz wird dabei durch einen eigenen Eintrag repräsentiert. Dieser besitzt eine eindeutige Identität, im Beispiel sind dies die Werte \texttt{1} und \texttt{2}. Die Einträge innerhalb der Super-Column werden nach dieser Identät sortiert gespeichert. Wird beim manuellen Erzeugen eines \texttt{TitanLabel} ein Primärschlüssel angegeben, so wird der entsprechende Attributwert als Identität verwendet. Die \texttt{WORKS\_WITH}-Beziehung lässt sich in beiden Richtungen traversieren und ist demzufolge an Start- und Zielknoten gespeichert. Kantenattribute werden dabei als assoziatives Array innerhalb des Wertes redundant hinterlegt.

Der Aufwand, die Nachbarschaft eines Knotens auszulesen, ist im Wesentlichen vom eingesetzten Speichersystem abhängig. Apache Cassandra basiert auf einer verteilten Hashtabelle und erlaubt den Zugriff auf einen Datensatz in konstanter Zeit\cite{Lakshman:2009:CSS:1582716.1582722}, während in BerkeleyDB der Aufwand in einem logarithmischen Verhältnis zur Anzahl der Einträge steht. Ein weiteres Defizit, welches sich aus der Verwendung von BerkeleyDB ergibt, ist die fehlende Unterstützung für den direkten Zugriff auf Attributwerte bzw. inzidente Kanten eines Knotens. Titan bildet das KCV-Modell auf das Key-Value-Modell von BerkeleyDB ab, indem es für jede Eintrag innerhalb eines Datensatzes einen eigenen Eintrag im Key-Value-Modell erzeugt.\footnote{Dies lässt sich im Quelltext unter \url{https://github.com/thinkaurelius/titan/blob/0.3.2/titan-core/src/main/java/com/thinkaurelius/titan/diskstorage/keycolumnvalue/keyvalue/OrderedKeyValueStoreAdapter.java\#L73} nachvollziehen.}
...
% BerkeleyDB
% eigene Caches
- multi-threaded Transaktions
	- Titan nutzt dabei die Datenstrukturen des \texttt{java.util.concurrent}-Packages für den paral
