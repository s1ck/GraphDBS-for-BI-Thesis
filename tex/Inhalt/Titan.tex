...
Abbildung \ref{fig:titan_example} zeigt eine mögliche Abbildung des Mitarbeiter-Projekt-Beispiels in Titan. Zu beachten sind dabei die zusätzlichen Einschränkungen an einzelnen Beziehungsarten. Für die Definition von Typen für Attributschlüssel und Kantenbezeichner erweitert Titan die Sprache um Hilfsfunktionen.  Zum Beispiel lässt sich ein \texttt{TitanKey} für den Attributschlüssel \texttt{name} wie folgt definieren:
...
% Berücksichtigung Kantenbezeichner
...
% Beispiele
	% Einschränkung Kantenbezeichner und Richtung (Whitelist)
	% Breiten- oder Tiefendurchlauf
	% Uniqueness Knoten / Kanten
	
% Selektion
	% Filter und Abbruchkriterium
	% Beispiel Alter > 30
	
% Projektion: Namen aller Mitarbeiter
...
% Bindung an Datenbasis (welche Datentypen? nur knoten und kanten?)
	% Konstanten im Muster (gebunden an Datenbasis)
	% Variablen im Muster (werden gefunden)
	% Datentypen für Konstanten und Variablen (Knoten und Kanten)
% Mustergraph wird durch Traversierung beschrieben (back)
	% komplexer als Cypher
%a.as(a)
%	.out(workswith).filter(type=Employee).as(b)
%	.out(workswith).filter(type=Employee).as(c)
%	.both().contains(a)
%	.back(b)
%	.out(responsiblefor).filter(type=Project).as(d)
% Variablen
